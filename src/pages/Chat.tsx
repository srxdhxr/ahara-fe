...
function Chat() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] max-w-4xl mx-auto px-4">
      ...
      {/* Search Box */}
      <div className="bg-white/50 backdrop-blur-sm rounded-[20px] p-4 clay-shadow mb-6">
        <div className="flex gap-3 items-center">
          <Search className="w-5 h-5 text-[#6B5B95] flex-shrink-0" strokeWidth={2} />
          <input
            type="text"
            value={question}
            ...
            className="flex-1 bg-transparent border-0 p-2 text-[#6B5B95] placeholder:text-[#8B7355] focus:outline-none"
            disabled={isLoading}
          />
          <button
            onClick={handleSearch}
            disabled={!question.trim() || isLoading}
            className="px-6 py-2 bg-gradient-to-br from-[#E8DEFF] to-[#D4E7FF] rounded-[16px] clay-shadow text-[#6B5B95] font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-105"
          >
